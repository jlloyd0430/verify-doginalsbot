...
client.once('ready', async () => {
  ...
  try {
    ...
    // Run the checkWallets function immediately after login
    await checkWallets();

    // Schedule the `checkWallets` function to run every 60 seconds
    setInterval(() => {
      console.log("Running scheduled `checkWallets`...");
...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    ...
    if (!interaction.replied) {
      await interaction.editReply('There was an error while processing the command.');
    }
  }
});
